...
export const startSession = async (
  ...
  // Logger yang sudah diperbaiki
  const logger = pino() as pino.Logger; // logger pino dengan level default
  ...
  const startSocket = async () => {
    ...
    let browser_end = Browsers.macOS("regE, Safari, 3.0");
    
    switch (BROWSERS_DISPLAY.BROWSERS) {
      case "macOS":
        browser_end = Browsers.macOS("regE, Safari, 3.0");
        break;
      default:
        browser_end = [BROWSERS_DISPLAY.BROWSERS ?? "regE", "Safari", "3.0"];
        break;
    }
